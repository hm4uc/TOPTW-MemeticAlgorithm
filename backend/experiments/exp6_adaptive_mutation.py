"""
Thí nghiệm 6: So sánh Adaptive-Lite Mutation vs Static Mutation.
...
Usage:
    cd backend
    py -m experiments.exp6_adaptive_mutation
...
NUM_RUNS = 5
OUTPUT_DIR = "experiments/results/exp6_adaptive_mutation"
...
def main():
    ...
    print("=" * 76)
    print("  THÍ NGHIỆM 6: ADAPTIVE-LITE MUTATION vs STATIC MUTATION")
    ...
    detail_path = os.path.join(args.output_dir, "exp6_detail.csv")
    detail_df.to_csv(detail_path, index=False)

    summary_df = pd.DataFrame([overall])
    summary_path = os.path.join(args.output_dir, "exp6_summary.csv")
